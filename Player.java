...
class Player {
    ...
    private static int stacksCount;
    private static int drawCardsLeft;    

    private static Team myPlayer;
    private static Game game;
    private static String action;

    private static List<Team> players;
    ...
    public static void main(String args[]) throws Exception {
        ...
        while (true){
            ...
            System.err.printf("my index = %s\n", myPlayerIndex);

            TreeMap<String, Card> takableCards = getTakableCards();

            for(String takeAction : takableCards.keySet()){
                System.err.println(takeAction);
            }
            ...
            if(pushAction != null){
                ...
                if(addAction != null){
                    ...
                    if(takeAction != null && myPlayer.getActionsLeft() > 0 && (!playedActions.contains(takeAction) || drawCardsLeft <= 0)){                
                        action = takeAction;
    ...
    private static void PARSE_STACKS(Scanner in) throws Exception{
    ...
    private static void PARSE_PLAYERS(Scanner in){
        
        players = new ArrayList<Team>();

        myPlayerIndex = in.nextInt();   // the index of your player
        playersCount = in.nextInt();    // the total number of player
        stacksCount = in.nextInt();        // the number of stacks pushed in the game
        drawCardsLeft = in.nextInt();
        ...
        for (int i = 0; i < playersCount; i++) {
            ...
            players.add(new Team(playerIndex, playerScore, actionsLeft));
    ...
    private static TreeMap<String, Card> getTakableCards(){

        TreeMap<String, Card> takableCards = new TreeMap<String, Card>();

        for(StackColor colorStack : game.colorStacks.values()){
            for(Card card : colorStack.getTakableCards()){
                takableCards.put(String.format("TAKE %s %s", colorStack.getID(), card.getHashCode()), card);
            }
        }

        for(StackSequence stackSequence : game.sequenceStacks.values()){
            for(Card card : stackSequence.getTakableCards()){
                takableCards.put(String.format("TAKE %s %s", stackSequence.getID(), card.getHashCode()), card);
            }       
        }
    ...
    private static class StackSequence extends CardStack{
    ...
    private static class StackColor extends CardStack{
